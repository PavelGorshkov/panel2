...
class SideBarWidget extends Menu {
    ...
    public function run()
    {
        ...
        $posDefaultAction = strpos($this->route, app()->controller->defaultAction);
        if ($posDefaultAction) {
            $this->noDefaultAction = rtrim(substr($this->route, 0, $posDefaultAction), '/');
        } else {
            $this->noDefaultAction = false;
        }
        $posDefaultRoute = strpos($this->route, app()->controller->module->defaultRoute);
        if ($posDefaultRoute) {
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            ...
            if ($route[0] !== '/' && app()->controller) {
                $route = ltrim(app()->controller->module->getUniqueId() . '/' . $route, '/');
            }
            $route = ltrim($route, '/');
